...
export default function InvoiceOrderList ({orders}){
    const [isLoading,setIsLoading] = useState(false);
    const order = orders.data
    var route_link = "invoice_order.index"
            ...
            <AuthenticatedLayout>
                    ...
                    <div className="container-fluid">
                        <div className="row">
                            <div className="col-sm-12">
                                <PageLink props={"Invoice Order Lists"}/>
                ...
                <Modal size="lg" show={show}  onHide={handleClose}>
                    ...
                    <form onSubmit={(e)=>submit(e)}>
                        <Modal.Body>
                            <Row className="mb-3">
                                <ModalFormTopLabel label="Customer name" type="text" xs="12" mdl="6" mdin="6" id="name" name="name" value={data?.name?data?.name:""} readOnly={true}/>

                                <ModalFormTopLabel label="invoice_amount" type="number" xs="12" mdl="6" mdin="6" id="invoice_amount" name="invoice_amount" value={data?.invoice_amount?data?.invoice_amount:""} onChange={(e) => setData('invoice_amount', e.target.value)} error={errors?.invoice_amount} readOnly={true}/>

                                <ModalFormTopLabel label="shipping_charge" type="number" xs="12" mdl="6" mdin="6" id="shipping_charge" name="shipping_charge" value={data?.shipping_charge?data?.shipping_charge:""} onChange={(e) => setData('shipping_charge', e.target.value)} error={errors?.shipping_charge}/>

                                <ModalFormTopLabel label="order note" type="text" xs="12" mdl="6" mdin="6" id="note" name="note" value={data?.note?data?.note:""} readOnly={true}/>


                                <ModalFormTopLabel className="text-danger" label="delivery date" type="date" xs="12" mdl="6" mdin="6" id="delivery_date" name="delivery_date" value={data.delivery_date?data.delivery_date:""} onChange={(e) => setData('delivery_date', e.target.value)} error={errors?.delivery_date} required={true}/>

                                <Col xs={12} md={6}>
                                    <label> {labelNameRSt("delivery status")} <strong className="text-danger ml-1"> *</strong></label>
                                    <select name="status" className="form-control" onChange={(e) => setData('status', e.target.value)} value={data?.status?data?.status:"1"} style={{fontSize:'13px',color:'black'}} required={true}>
                                ...
                                {orderLogs&&orderLogs.map((row,i)=>(
                                    <Col xs={12} md={12} key={i}>
                                        Comment : {i+1} <input className="form-control mt-2 mb-2" type="text" value={row?.note?row?.note:""} readOnly={true}/>
                                    </Col>
                                ))}
